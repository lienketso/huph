    ...
    <div class="row">
        <div class="col-lg-4">
            <input class="form-control"
                   name="meta[meta_tree_thuoc][title]"
                   type="text"
                   value="{{($postModel->getPostMetaJson('meta_tree_thuoc','title',$data->id)=='null')) ? $setting->getSettingMeta('form_key_30_'.$lang) : $postModel->getPostMetaJson('meta_tree_thuoc','title',$data->id}}"
                   placeholder="Tiêu đề trường (hiển thị ở trường search)">
        </div>
        <div class="col-lg-8">
            <input class="form-control"
                   name="meta[meta_tree_thuoc][name]"
                   type="text"
                   value="{{old('meta_tree_thuoc',$postModel->getPostMetaJson('meta_tree_thuoc','name',$data->id))}}"
