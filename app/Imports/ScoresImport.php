...
class ScoresImport implements ToModel
{
    ...
    */
    ...
    public function model(array $row)
    {
        ...
        // Kiểm tra nếu cccd_number đã tồn tại
        $existingScore = Scores::where('cccd_number', $row[1])->first();
        if ($existingScore) {
            $existingScore->update([
                ...
                'priority_score_one' => (!empty($row[8]) ? $row[8] : "0"), // Điểm ưu tiên môn 1
                ...
                'priority_score_two' => (!empty($row[12]) ? $row[12] : "0"), //Điểm ưu tiên môn 2
            ...
            return $existingScore; // Trả về bản ghi đã cập nhật
        }else{
            return new Scores([
                'identification_number'=> (!empty($row[0]) ? $row[0] : "0"), // số báo danh
